...
public class Bc6EncoderTests(ITestOutputHelper output)
{
	[Theory]
	...
	public void PackMode3()
	{
		...
		var block = Bc6Block.PackType3(ep0, ep1, indices, new ArgumentOutOfRangeException());
	...
	internal void EncodeAllModesUnsigned(Bc6BlockType type)
	{
		...
		if(type.HasSubsets())
		{
			var indexBlock = Bc6Encoder.CreateClusterIndexBlock(testBlock, out var numClusters);
	...
	public void Encode()
	{
		...
		for (var i = 0; i < blocks.Length; i++)
		{
			...
			if (!(error > 0.06) && i != 14749) continue;
			Debugger.Break();
