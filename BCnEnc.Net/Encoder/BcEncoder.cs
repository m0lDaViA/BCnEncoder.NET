...
public class BcEncoder
{
	...
	/// </summary>
	public EncoderInputOptions InputOptions { get; } = new();

	/// <summary>
	/// The output options of the encoder.
	/// </summary>
	public EncoderOutputOptions OutputOptions { get; } = new();

	/// <summary>
	/// The encoder options.
	/// </summary>
	public EncoderOptions Options { get; } = new();
	...
	public Task EncodeToStreamAsync(ReadOnlyMemory<byte> input, int width, int height, PixelFormat format,
		Stream outputStream, CancellationToken token = default)
	{
		return Task.Run(() =>
		{
	...
	public Task<KtxFile> EncodeToKtxAsync(ReadOnlyMemory<byte> input, int width, int height, PixelFormat format,
	...
	public Task<DdsFile> EncodeToDdsAsync(ReadOnlyMemory<byte> input, int width, int height, PixelFormat format,
		CancellationToken token = default)
	{
	...
	public Task<byte[][]> EncodeToRawBytesAsync(ReadOnlyMemory<byte> input, int width, int height,
		PixelFormat format, CancellationToken token = default)
	{
	...
	public Task<byte[]> EncodeToRawBytesAsync(ReadOnlyMemory<byte> input, int width, int height, PixelFormat format,
		int mipLevel, CancellationToken token = default)
	{
	...
	public byte[] EncodeToRawBytes(ReadOnlySpan<byte> input, int width, int height, PixelFormat format,
		int mipLevel, out int mipWidth, out int mipHeight)
	{
	...
	public int GetBlockSize()
	{
		...
		if (compressedEncoder != null) return compressedEncoder.GetBlockSize();
		var hdrEncoder = GetFloatBlockEncoder(OutputOptions.Format);

		return hdrEncoder?.GetBlockSize() ?? throw new NotSupportedException(
			$"This format is either not supported or does not use block compression: {OutputOptions.Format}");
	}
	...
	private KtxFile EncodeToKtxInternalHdr(ReadOnlyMemory2D<ColorRgbFloat> input, CancellationToken token)
	{
		...
		for (var mip = 0; mip < numMipMaps; mip++)
		{
			...
			context.Progress.SetProcessedBlocks(mipChain.Take(mip + 1)
				.Sum(x => ImageToBlocks.CalculateNumOfBlocks(x.Width, x.Height)));
	...
	private DdsFile EncodeToDdsInternalHdr(ReadOnlyMemory2D<ColorRgbFloat> input, CancellationToken token)
	{
		...
		var output = new DdsFile(ddsHeader, dxt10Header);
		...
		for (var mip = 0; mip < numMipMaps; mip++)
		{
			...
			var encoded = compressedEncoder.Encode(blocks, blocksWidth, blocksHeight, OutputOptions.Quality, context);
	...
	private byte[][] EncodeToRawInternalHdr(ReadOnlyMemory2D<ColorRgbFloat> input, CancellationToken token)
	{
		...
		IBcBlockEncoder<RawBlock4X4RgbFloat> compressedEncoder;
	...
	private byte[] EncodeToRawInternalHdr(ReadOnlyMemory2D<ColorRgbFloat> input, int mipLevel, out int mipWidth,
		out int mipHeight, CancellationToken token)
	{
	...
	private void EncodeCubeMapToStreamInternalHdr(ReadOnlyMemory2D<ColorRgbFloat> right,
		ReadOnlyMemory2D<ColorRgbFloat> left, ReadOnlyMemory2D<ColorRgbFloat> top,
		ReadOnlyMemory2D<ColorRgbFloat> down,
	...
	private KtxFile EncodeCubeMapToKtxInternalHdr(ReadOnlyMemory2D<ColorRgbFloat> right,
		ReadOnlyMemory2D<ColorRgbFloat> left, ReadOnlyMemory2D<ColorRgbFloat> top,
		ReadOnlyMemory2D<ColorRgbFloat> down,
		ReadOnlyMemory2D<ColorRgbFloat> back, ReadOnlyMemory2D<ColorRgbFloat> front, CancellationToken token)
	{
		...
		if (compressedEncoder != null)
		{
			...
			for (var face = 0; face < faces.Length; face++)
			{
				...
				{
					var blocks = ImageToBlocks.ImageTo4X4(mipChain[mipLevel], out var blocksWidth,
						out var blocksHeight);
					var encoded = compressedEncoder.Encode(blocks, blocksWidth, blocksHeight, OutputOptions.Quality,
						context);
	...
	private DdsFile EncodeCubeMapToDdsInternalHdr(ReadOnlyMemory2D<ColorRgbFloat> right,
		ReadOnlyMemory2D<ColorRgbFloat> left, ReadOnlyMemory2D<ColorRgbFloat> top,
		ReadOnlyMemory2D<ColorRgbFloat> down,
		ReadOnlyMemory2D<ColorRgbFloat> back, ReadOnlyMemory2D<ColorRgbFloat> front, CancellationToken token)
	{
	...
	private void EncodeBlockInternalHdr(ReadOnlySpan2D<ColorRgbFloat> input, Stream outputStream)
	{
		...
		else
		{
			throw new NotSupportedException(
				$"This Format is not supported for single block encoding: {OutputOptions.Format}");
		}
	}
	...
	private KtxFile EncodeToKtxInternal(ReadOnlyMemory2D<ColorRgba32> input, CancellationToken token)
	{
		...
		// Calculate total blocks
		var totalBlocks = isCompressedFormat
			? mipChain.Sum(m => ImageToBlocks.CalculateNumOfBlocks(m.Width, m.Height))
			: mipChain.Sum(m => m.Width * m.Height);
		context.Progress = new OperationProgress(Options.Progress, totalBlocks);
	...
	private byte[] EncodeToRawInternal(ReadOnlyMemory2D<ColorRgba32> input, int mipLevel, out int mipWidth,
		out int mipHeight, CancellationToken token)
	{
		mipLevel = Math.Max(0, mipLevel);
		...
		// Calculate total blocks
		var totalBlocks = isCompressedFormat
			? ImageToBlocks.CalculateNumOfBlocks(mipChain[mipLevel].Width, mipChain[mipLevel].Height)
			: mipChain[mipLevel].Width * mipChain[mipLevel].Height;
		context.Progress = new OperationProgress(Options.Progress, totalBlocks);
	...
	private void EncodeCubeMapToStreamInternal(ReadOnlyMemory2D<ColorRgba32> right,
		ReadOnlyMemory2D<ColorRgba32> left, ReadOnlyMemory2D<ColorRgba32> top, ReadOnlyMemory2D<ColorRgba32> down,
	...
	private KtxFile EncodeCubeMapToKtxInternal(ReadOnlyMemory2D<ColorRgba32> right,
		ReadOnlyMemory2D<ColorRgba32> left, ReadOnlyMemory2D<ColorRgba32> top, ReadOnlyMemory2D<ColorRgba32> down,
	...
	private DdsFile EncodeCubeMapToDdsInternal(ReadOnlyMemory2D<ColorRgba32> right,
		ReadOnlyMemory2D<ColorRgba32> left, ReadOnlyMemory2D<ColorRgba32> top, ReadOnlyMemory2D<ColorRgba32> down,
		ReadOnlyMemory2D<ColorRgba32> back, ReadOnlyMemory2D<ColorRgba32> front, CancellationToken token)
	{
		DdsFile output;
	...
	private IBcBlockEncoder<RawBlock4X4Rgba32> GetRgba32BlockEncoder(CompressionFormat format)
	{
		return format switch
		{
			...
			CompressionFormat.R => null,
			CompressionFormat.Rg => null,
			CompressionFormat.Rgb => null,
			CompressionFormat.Rgba => null,
			CompressionFormat.Bgra => null,
			CompressionFormat.Bc6U => null,
			CompressionFormat.Bc6S => null,
			CompressionFormat.Unknown => null,
			_ => null
		};
	}
	...
	private IBcBlockEncoder<RawBlock4X4RgbFloat> GetFloatBlockEncoder(CompressionFormat format)
	{
		return format switch
		{
			...
			CompressionFormat.Rgba => null,
			CompressionFormat.Bgra => null,
			CompressionFormat.Bc1 => null,
			CompressionFormat.Bc1WithAlpha => null,
			CompressionFormat.Bc2 => null,
			CompressionFormat.Bc3 => null,
			CompressionFormat.Bc4 => null,
			CompressionFormat.Bc5 => null,
			CompressionFormat.Bc7 => null,
			CompressionFormat.Atc => null,
			CompressionFormat.AtcExplicitAlpha => null,
			CompressionFormat.AtcInterpolatedAlpha => null,
			CompressionFormat.Unknown => null,
			_ => (IBcBlockEncoder<RawBlock4X4RgbFloat>)null
		};
	}
