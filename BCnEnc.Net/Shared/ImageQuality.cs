...
public class ImageQuality
{
	...
	/// <param name="countAlpha"></param>
	/// <returns></returns>
	/// <exception cref="ArgumentException"></exception>
	public static float PeakSignalToNoiseRatio(ReadOnlySpan<ColorRgba32> original, ReadOnlySpan<ColorRgba32> other,
		bool countAlpha = true)
	{
	...
	public static float CalculateLogRmse(ReadOnlySpan<ColorRgbFloat> original, ReadOnlySpan<ColorRgbFloat> other)
	{
		...
		for (var i = 0; i < original.Length; i++)
		{
			...
			var dr = Math.Sign(other[i].r) * MathF.Log(1 + MathF.Abs(other[i].r)) -
			         Math.Sign(original[i].r) * MathF.Log(1 + MathF.Abs(original[i].r));
			var dg = Math.Sign(other[i].g) * MathF.Log(1 + MathF.Abs(other[i].g)) -
			         Math.Sign(original[i].g) * MathF.Log(1 + MathF.Abs(original[i].g));
			var db = Math.Sign(other[i].b) * MathF.Log(1 + MathF.Abs(other[i].b)) -
			         Math.Sign(original[i].b) * MathF.Log(1 + MathF.Abs(original[i].b));
	...
	/// <returns></returns>
	/// <exception cref="ArgumentException"></exception>
	public static float PeakSignalToNoiseRatioLuminance(ReadOnlySpan<ColorRgba32> original,
		ReadOnlySpan<ColorRgba32> other, bool countAlpha = true)
	{
		if (original.Length != other.Length) {
			throw new ArgumentException("Both spans should be the same length");
		}
