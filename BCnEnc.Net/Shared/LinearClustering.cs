...
internal static class LinearClustering
{
	...
	{
		private float l = labxy.l;
		private float a = labxy.a;
		private float b = labxy.b;
		...
		Span<ClusterCenter> previousCenters, float s, ref Span<ClusterCenter> clusterCenters)
	{
		...
		{
			if (clusterCenters[i].count <= 0) continue;
	...
	private static ClusterCenter[] InitialClusterCenters(int width, int height, int clusters, float s, IReadOnlyList<LabXy> labXys)
	{
	...
	private static int[] EnforceConnectivity(int[] oldLabels, int width, int height, int clusters)
	{
		...
		for (var y = 0; y < height; ++y)
		{
			for (var x = 0; x < width; ++x)
			{
				...
				if (newLabels[xyIndex] >= 0) continue;
				...
				for (var c = 0; c < clusterX.Count; ++c)
				{
					for (var i = 0; i < neighborX.Length; ++i)
					{
						...
						var nI = nX + nY * width;
						if (nX >= width || nX < 0 || nY >= height || nY < 0) continue;
						if (newLabels[nI] != -1 || label != oldLabels[nI]) continue;
						clusterX.Add(nX);
						clusterY.Add(nY);
						newLabels[nI] = label;
					}
