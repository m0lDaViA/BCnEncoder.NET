...
internal static class MipMapper
{
	...
	public static ReadOnlyMemory2D<ColorRgba32>[] GenerateMipChain(ReadOnlyMemory<ColorRgba32> input, int width,
		int height, ref int numMipMaps) => GenerateMipChain(input.AsMemory2D(height, width), ref numMipMaps);
	...
	/// <param name="numMipMaps">The number of mipmaps to generate.</param>
	/// <returns>Will generate as many mipmaps as possible until a mipmap of 1x1 is reached for <paramref name="numMipMaps"/> 0 or smaller.</returns>
	public static ReadOnlyMemory2D<ColorRgbFloat>[] GenerateMipChain(ReadOnlyMemory<ColorRgbFloat> input, int width,
		int height, ref int numMipMaps) => GenerateMipChain(input.AsMemory2D(height, width), ref numMipMaps);
